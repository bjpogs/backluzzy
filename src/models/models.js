...
Users.getshopproducts = (result) => {
    con.query('select * from producttbl where product_status = 0 order by if(product_category RLIKE "[a-z]", 1 , 2), product_subcategory asc', (err, res) => {
        if (err) result(null, err);
        else result(null, res);
    })
}
...
// update produt
Users.updateshopproduct = (id, data, result) => {
    console.log('id',id)
...
Users.buildselect = (result) => {
    con.query('select * from selecttbl order by name asc', (err, res) => {
        if (err) result(null, err);
        else result(null, res);
    })
}
...
Users.myorders = (id, result) => {
	con.query('select * from statustbl st inner join buildcaketbl bt on st.order_id = bt.order_id where st.user_id = ? order by placeddate desc', id, (err, res) => {
		if (err) result(null, err);
        else result(null, res); 
	})
}
Users.myreserveorders = (id, result) => {
	con.query('select * from statustbl st inner join reservationtbl bt on st.order_id = bt.order_id where st.user_id = ? order by placeddate desc', id, (err, res) => {
		if (err) result(null, err);
        else result(null, res); 
	})
}
Users.myshoporders = (id, result) => {
	con.query('select * from statustbl st inner join ordertbl bt on st.order_id = bt.order_id inner join producttbl pt on bt.product_id = pt.product_id where st.user_id = ? order by placeddate desc', id, (err, res) => {
		if (err) result(null, err);
        else result(null, res); 
	})
}
